...
export const Login = () => {
  ...
  const _submit = () => {
    if (!loading) loginMutation({ variables: { loginInput: getValues() } });
  };
      ...
      <div className="bg-white shadow-2xl rounded-lg w-full max-w-3xl mx-5 flex justify-between">
        <div className="hidden sm:block sm:w-7/12 bg-gradient-to-tr from-green-400 to-blue-400  flex justify-center items-center rounded-tl-lg rounded-bl-lg">
          ...
          <div className="w-full h-full flex flex-col justify-center text-white text-right p-6">
            ...
            <span className="mt-4 text-medium">Just Subscribe! Deliver all podcasts to your ears.</span>
          </div>
        </div>
        <div className="w-full sm:w-5/12 py-16">
          ...
          <form onSubmit={handleSubmit(_submit)} className="w-full flex flex-col px-14">
            ...
            {errors.password?.type === "minLength" && <FormError errorMessage="Password must be more than 10 characters" />}

            <Button className="mt-12" canClick={formState.isValid} loading={loading} actionText="Login" />
